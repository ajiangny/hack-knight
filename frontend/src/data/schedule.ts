...
export const scheduleHours: number[] = Array.from(
  { length: 24 - 6 + 1 },
  (_, i) => 6 + i
);
...
export function formatFullTime(hour: number): string {
...
export function formatShortTime(hour: number): string {
  const h = Math.floor(hour);
  const m = Math.round((hour - h) * 60);
  const period = h >= 12 ? 'PM' : 'AM';
  const displayH = h % 12 || 12;
...
export function hourToSlot(decimalHour: number): number {
