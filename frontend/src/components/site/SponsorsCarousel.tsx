...
function SponsorCard({ sponsor, duplicate }: { sponsor: Sponsor; duplicate: boolean }) {
  const className = "card-sponsor shrink-0 w-36 h-24 sm:w-56 sm:h-36 flex items-center justify-center p-3 sm:p-8";
  const logo = <img src={sponsor.logo} alt={sponsor.name} className="max-w-[85%] max-h-14 sm:max-h-20 object-contain" />;
