...
//
// KEEP IN SYNC with backend/src/lib/schools.ts — the backend rejects any
// school not in its copy, so the two lists must come from the same CSV.
//
// ~13k entries: import this module lazily (the school combobox does) so it
// becomes its own chunk instead of weighing down the initial bundle.
